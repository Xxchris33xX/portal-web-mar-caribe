let tableUsers;
let tableHistory;
const userID = document.getElementById('idHistoryuser');

$.extend( true, $.fn.dataTable.defaults, {
    "language": {
        "decimal": ",",
        "thousands": ".",
        "info": "Mostrando registros del _START_ al _END_ de un total de _TOTAL_ registros",
        "infoEmpty": "Mostrando registros del 0 al 0 de un total de 0 registros",
        "infoPostFix": "",
        "infoFiltered": "(filtrado de un total de _MAX_ registros)",
        "loadingRecords": "Cargando...",
        "lengthMenu": "Mostrar _MENU_ registros",
        "paginate": {
            "first": "Primero",
            "last": "Último",
            "next": "Siguiente",
            "previous": "Anterior"
        },
        "processing": "Procesando...",
        "search": "Buscar:",
        "searchPlaceholder": "Término de búsqueda",
        "zeroRecords": "No se encontraron resultados",
        "emptyTable": "Ningún dato disponible en esta tabla",
        "aria": {
            "sortAscending":  ": Activar para ordenar la columna de manera ascendente",
            "sortDescending": ": Activar para ordenar la columna de manera descendente"
        },
        //only works for built-in buttons, not for custom buttons
        "buttons": {
            "create": "Nuevo",
            "edit": "Cambiar",
            "remove": "Borrar",
            "copy": "Copiar",
            "csv": "fichero CSV",
            "excel": "tabla Excel",
            "pdf": "documento PDF",
            "print": "Imprimir",
            "colvis": "Visibilidad columnas",
            "collection": "Colección",
            "upload": "Seleccione fichero...."
        },
        "select": {
            "rows": {
                _: '%d filas seleccionadas',
                0: 'clic fila para seleccionar',
                1: 'una fila seleccionada'
            }
        }
    }           
} );        

document.addEventListener('DOMContentLoaded',function(){
    tableUsers = new DataTable( "#tableUsers" , {
        "aProcessing":true,
        "aServerSide":true,
        /*"language": {
            "url": "//cdn.datatables.net/plug-ins/1.10.20/i18n/Spanish.json",
        },*/
        "ajax":{
            "url" : host + path +"/con_user/JSON/JSON",
            "dataSrc" : ""
        },
        "columns":[
            {"data":"nombre"},
            {"data":"apellido"},
            {"data":"nom_usuario"},
            {"data":"cedula"},
            {"data":"nom_rol"},
            {
                "data": null,
                "defaultContent": "<button type='button' onCLick='openModalShowEdit();' class='showInfoUser btn btn-success' Onmouseover='popUp();' data-toggle='tooltip' data-placement='top' title='Información de contacto'><i class='bx bxs-user-badge'></i></button>"+ " " +"<button class='historyUser btn btn-info' Onmouseover='popUp();' onCLick='openModalHistory();' data-toggle='tooltip' data-placement='top' title='Historial'><i class='bx bx-time'></i></button>"+ " " +"<button title='Editar Usuario' onCLick='openModalEdit();' type='button' onCLick='' class='editUser btn btn-success' Onmouseover='popUp();' data-toggle='tooltip' data-placement='top' ><i class='bx bxs-wrench'></i></button>"+ " " +"<button Onmouseover='popUp();' onClick='openModalDeleteUser();' type='button' class='deleteUser btn btn-danger' data-toggle='tooltip' data-placement='top' title='Eliminar el usuario'><i class='bx bx-x'></i></button>"
            }
        ],
        "columnDefs": [
            { targets: [0, 1, 2, 3, 4, 5], visible: true},
            { targets: '_all', visible: false }
        ],
        "resonsieve":"true",
        "bDestroy": true,
        "iDisplayLength": 10,
        "order":[[0,"desc"]],
        "dom": "lBfrtip",
        "buttons":[
            {
                "extend":"copyHtml5",
                "text":"<i class='bx bx-copy-alt' Onmouseover='popUp();' data-toggle='tooltip' data-placement='top' title='Copiar en el portapapeles'></i>",
                "className":"btn btn-warning",
            },
            {
                "extend":"excelHtml5",
                "text":"<i class='bx bx-table' Onmouseover='popUp();' data-toggle='tooltip' data-placement='top' title='Exportar Excel'></i>",
                "className":"btn btn-success"
            },
            {
                "extend":"pdfHtml5",
                "text":"<i class='bx bxs-file-pdf'  Onmouseover='popUp();' data-toggle='tooltip' data-placement='top' title='Exportar PDF'></i>",
                "className":"btn btn-danger",
                "title": 'Usuarios Activos',
                "alignment":"center",
                "exportOptions": {
                    columns: [0,1,2,3,4] 
                },
                customize: function(doc) {
                  doc.content.splice( 0, 1, {
                       margin: [ 0, 0, 0, 12 ],
                       alignment: 'center',
                       image: 'data:image/png;base64,'+encode,
                   } );
                   doc.content[1].fontSize = 16;
                   doc.content[2].table.widths = 
                   Array(doc.content[2].table.body[0].length + 1).join('*').split('');
                   let rowCount = tableUsers.rows.length;
                   for (i = 0; i < rowCount; i++) {
                       doc.content[2].table.body[i][2].alignment = 'right';
                   };
                   doc.defaultStyle.alignment = 'center';
                 },
                 "messageTop": 'Usuarios activos'
                 
            },
            {
                "text":"<i class='bx bx-user-check' Onmouseover='popUp();' Onclick='popUpHidden();' data-toggle='tooltip' data-placement='top' title='Ver usuarios activos'></i>",
                "className":"btn btn-warning",
                "action":function(){
                    showActiveUsers();
                }
            },
            {
                "text":"<i class='bx bx-user-x' Onmouseover='popUp();' data-toggle='tooltip' Onclick='popUpHidden();' data-placement='top' title='Ver usuarios inactivos'></i>",
                "className":"btn btn-warning",
                "action":function(){
                    showInactiveUsers();
                }
            }
        ]
    })
},false);

// MODAL MOSTRAR INFORMACIÓN
$("#tableUsers tbody").on(
    "click",
    "button.showInfoUser",
    async function()
    {
      let dataRow = tableUsers.row($(this).parents("tr")).data();
      let inputDireccion = $("#Direccion").val(dataRow.direccion);
      let inputTelefono = $("#Telefono").val(dataRow.telefono);
      let inputCorreo = $("#Correo").val(dataRow.correo);
    }
);

//MODAL MOSTRAR 

$("#tableUsers tbody").on(
    "click",
    "button.editUser",
    async function()
    {
      $('[data-toggle="tooltip"]').tooltip('hide');
      let dataRow = tableUsers.row($(this).parents("tr")).data();
      $("#Nombre").val(dataRow.nombre);
      $("#Apellido").val(dataRow.apellido);
      $("#Username").val(dataRow.nom_usuario);
      $("#Cedula").val(dataRow.cedula);
      $("#Direccion2").val(dataRow.direccion);
      $("#id_user").val(dataRow.id_usuario);
      $("#id_personal").val(dataRow.id_personal);
      $("#Telefono2").val(dataRow.telefono);
      $("#Correo2").val(dataRow.correo);
      $("#pass1").val(dataRow.contrasenia);
      $("#pass2").val(dataRow.contrasenia);
    }
);

$("#tableUsers tbody").on(
    "click",
    "button.deleteUser",
    async function()
    {
      let dataRow = tableUsers.row($(this).parents("tr")).data();
      $("#userName").html(dataRow.nom_usuario),
      $("#userNameToDelete").val(dataRow.nom_usuario),
      $("#id_UserToDelete").val(dataRow.id_usuario),
      $("#deleteMessage").val('Se eliminó un usuario')
    }
);

//ABRE MODAL INFORMACIÓN

function openModalShowEdit(){
    $('#showInfoUser').modal('show')
};

function openModalEdit(){
    $('#modalEditUser').modal('show')
};

function openModalDeleteUser(){
    $('#deleteUser').modal('show')
};

function openModalActiveUser(){
    $('#activeUser').modal('show')
};

function popUpHidden() {
    $('[data-toggle="tooltip"]').tooltip('hide')
}

$("#tableUsers tbody").on(
    "click",
    "button.historyUser",
     async function()
    {
        $('[data-toggle="tooltip"]').tooltip('hide');
        let dataRow = tableUsers.row($(this).parents("tr")).data();
        let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
        let ajaxetUser = host + path +"/con_user/getHistorybyUserId/?id="+dataRow.id_usuario;
        request.open("GET",ajaxetUser,true);
        request.send();

        request.onreadystatechange = function(){
            if(request.readyState == 4 && request.status === 200){
                let objData = JSON.parse(request.responseText);
                tableHistory = new DataTable('#showHistoryUser', {
                    "aProcessing":true,
                    "aServerSide":true,
                    "data":objData,
                    "columns":[
                        {"data":"nom_usuario"},
                        {"data":"mensaje"},
                        {"data":"nombre_entidad"},
                        {"data":"fecha"},
                        {"data":"hora"}
                    ],
                    "resonsieve":"true",
                    "bDestroy": true,
                    "iDisplayLength": 5,
                    "columnDefs": [{ type: 'date-eu' ,'targets': 3}],
                    "order":[[3,"desc"],[4,"desc"]]
                })
                $('#historyUser').modal('show');
                
            }
        }

    }
);


function openModalHistory(){
    //$('#historyUser').modal('show');
};

function popUp(){
    $('[data-toggle="tooltip"]').tooltip()
}

function showActiveUsers(){
    tableUsers.ajax.url(host+path+"/con_user/JSON/JSON").load();
    tableUsers = new DataTable( "#tableUsers" , {
        "aProcessing":true,
        "aServerSide":true,
        /*"language": {
            "url": "//cdn.datatables.net/plug-ins/1.10.20/i18n/Spanish.json",
        },*/
        "ajax":{
            "url" : host + path +"/con_user/JSON/JSON",
            "dataSrc" : ""
        },
        "columns":[
            {"data":"nombre"},
            {"data":"apellido"},
            {"data":"nom_usuario"},
            {"data":"cedula"},
            {"data":"nom_rol"},
            {
                "data": null,
                "defaultContent": "<button type='button' onCLick='openModalShowEdit();' class='showInfoUser btn btn-success' Onmouseover='popUp();' data-toggle='tooltip' data-placement='top' title='Información de contacto'><i class='bx bxs-user-badge'></i></button>"+ " " +"<button class='historyUser btn btn-info' Onmouseover='popUp();' onCLick='openModalHistory();' data-toggle='tooltip' data-placement='top' title='Historial'><i class='bx bx-time'></i></button>"+ " " +"<button title='Editar Usuario' onCLick='openModalEdit();' type='button' onCLick='' class='editUser btn btn-success' Onmouseover='popUp();' data-toggle='tooltip' data-placement='top' ><i class='bx bxs-wrench'></i></button>"+ " " +"<button Onmouseover='popUp();' onClick='openModalDeleteUser();' type='button' class='deleteUser btn btn-danger' data-toggle='tooltip' data-placement='top' title='Eliminar el usuario'><i class='bx bx-x'></i></button>"
            }
        ],
        "columnDefs": [
            { targets: [0, 1, 2, 3, 4, 5], visible: true},
            { targets: '_all', visible: false }
        ],
        "resonsieve":"true",
        "bDestroy": true,
        "iDisplayLength": 10,
        "order":[[0,"desc"]],
        "dom": "lBfrtip",
        "buttons":[
            {
                "extend":"copyHtml5",
                "text":"<i class='bx bx-copy-alt' Onmouseover='popUp();' data-toggle='tooltip' data-placement='top' title='Copiar en el portapapeles'></i>",
                "className":"btn btn-warning",
            },
            {
                "extend":"excelHtml5",
                "text":"<i class='bx bx-table' Onmouseover='popUp();' data-toggle='tooltip' data-placement='top' title='Exportar Excel'></i>",
                "className":"btn btn-success"
            },
            {
                "extend":"pdfHtml5",
                "text":"<i class='bx bxs-file-pdf'  Onmouseover='popUp();' data-toggle='tooltip' data-placement='top' title='Exportar PDF'></i>",
                "className":"btn btn-danger",
                "title": 'Usuarios Activos',
                "exportOptions": {
                    columns: [0,1,2,3,4] 
                },
                customize: function(doc) {
                  doc.content.splice( 0, 1, {
                       margin: [ 0, 0, 0, 12 ],
                       alignment: 'center',
                       image: 'data:image/png;base64,'+encode,
                   } );
                    doc.content[1].fontSize = 16;
                   doc.content[2].table.widths = 
                   Array(doc.content[2].table.body[0].length + 1).join('*').split('');
                   let rowCount = tableUsers.rows.length;
                   for (i = 0; i < rowCount; i++) {
                       doc.content[2].table.body[i][2].alignment = 'right';
                   };
                   doc.defaultStyle.alignment = 'center';
                 },
                 "messageTop": 'Usuarios Activos'
            },
            {
                "text":"<i class='bx bx-user-check' Onmouseover='popUp();' Onclick='popUpHidden();' data-toggle='tooltip' data-placement='top' title='Ver usuarios activos'></i>",
                "className":"btn btn-warning",
                "action":function(){
                    showActiveUsers();
                }
            },
            {
                "text":"<i class='bx bx-user-x' Onmouseover='popUp();' Onclick='popUpHidden();' data-toggle='tooltip' data-placement='top' title='Ver usuarios activos'></i>",
                "className":"btn btn-warning",
                "action":function(){
                    showInactiveUsers();
                }
            }
        ]
    })
}

function showInactiveUsers(){
    tableUsers.ajax.url(host+path+"/con_user/inactivoJSON/inactivoJSON").load();
    tableUsers = new DataTable( "#tableUsers" , {
        "aProcessing":true,
        "aServerSide":true,
        /*"language": {
            "url": "//cdn.datatables.net/plug-ins/1.10.20/i18n/Spanish.json",
        },*/
        "ajax":{
            "url" : host + path +"/con_user/inactivoJSON/inactivoJSON",
            "dataSrc" : ""
        },
        "columns":[
            {"data":"nombre"},
            {"data":"apellido"},
            {"data":"nom_usuario"},
            {"data":"cedula"},
            {"data":"nom_rol"},
            {
                "data": null,
                "defaultContent": "<button type='button' onCLick='openModalShowEdit();' class='showInfoUser btn btn-success' Onmouseover='popUp();' data-toggle='tooltip' data-placement='top' title='Información de contacto'><i class='bx bxs-user-badge'></i></button>"+ " " +"<button class='historyUser btn btn-info' Onmouseover='popUp();' onCLick='openModalHistory();' data-toggle='tooltip' data-placement='top' title='Historial'><i class='bx bx-time'></i></button>"+ " " +"<button title='Editar Usuario' onCLick='openModalEdit();' type='button' onCLick='' class='editUser btn btn-success' Onmouseover='popUp();' data-toggle='tooltip' data-placement='top' ><i class='bx bxs-wrench'></i></button>"+ " " +"<button Onmouseover='popUp();' onClick='openModalActiveUser();' type='button' class='activeUser btn btn-primary' data-toggle='tooltip' data-placement='top' title='Activar usuario'><i class='bx bx-check'></i></button>"
            }
        ],
        "columnDefs": [
            { targets: [0, 1, 2, 3, 4, 5], visible: true},
            { targets: '_all', visible: false }
        ],
        "resonsieve":"true",
        "bDestroy": true,
        "iDisplayLength": 10,
        "order":[[0,"desc"]],
        "dom": "lBfrtip",
        "buttons":[
            {
                "extend":"copyHtml5",
                "text":"<i class='bx bx-copy-alt' Onmouseover='popUp();' data-toggle='tooltip' data-placement='top' title='Copiar en el portapapeles'></i>",
                "className":"btn btn-warning",
            },
            {
                "extend":"excelHtml5",
                "text":"<i class='bx bx-table' Onmouseover='popUp();' data-toggle='tooltip' data-placement='top' title='Exportar Excel'></i>",
                "className":"btn btn-success"
            },
            {
                "extend":"pdfHtml5",
                "text":"<i class='bx bxs-file-pdf'  Onmouseover='popUp();' data-toggle='tooltip' data-placement='top' title='Exportar PDF'></i>",
                "className":"btn btn-danger",
                "title": 'Usuarios Inactivos',
                "alignment":"center",
                "exportOptions": {
                    columns: [0,1,2,3,4] 
                },
                customize: function(doc) {
                  doc.content.splice( 0, 1, {
                       margin: [ 0, 0, 0, 12 ],
                       alignment: 'center',
                       image: 'data:image/png;base64,'+encode,
                   } );
                   doc.content[1].fontSize = 16;
                   doc.content[2].table.widths = 
                   Array(doc.content[2].table.body[0].length + 1).join('*').split('');
                   let rowCount = tableUsers.rows.length;
                   for (i = 0; i < rowCount; i++) {
                       doc.content[2].table.body[i][2].alignment = 'right';
                   };
                   doc.defaultStyle.alignment = 'center';
                 },
                 "messageTop": 'Usuarios Inactivos'
            },
            {
                "text":"<i class='bx bx-user-check' Onmouseover='popUp();' Onclick='popUpHidden();' data-toggle='tooltip' data-placement='top' title='Ver usuarios activos'></i>",
                "className":"btn btn-warning",
                "action":function(){
                    showActiveUsers();
                }
            },
            {
                "text":"<i class='bx bx-user-x' Onmouseover='popUp();' Onclick='popUpHidden();' data-toggle='tooltip' data-placement='top' title='Ver usuarios inactivos'></i>",
                "className":"btn btn-warning",
                "action":function(){
                    showInactiveUsers();
                }
            }
        ]
    })
}

$("#tableUsers tbody").on(
    "click",
    "button.activeUser",
    async function()
    {
      let dataRow = tableUsers.row($(this).parents("tr")).data();
      $("#userNameActive").html(dataRow.nom_usuario),
      $("#userNameToActive").val(dataRow.nom_usuario),
      $("#id_UserToActive").val(dataRow.id_usuario),
      $("#activeMessage").val('Se activó un usuario')
    }
);

let encode = '';