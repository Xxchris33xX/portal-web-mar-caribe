<?php
class usuario {

    private $conn;
    private $n;
    private $r;

    public function __construct(){
    try {
        $this -> conn = new PDO (
        "pgsql:host=localhost; 
        dbname=TiendaMarCaribeCenter", 
        "admin", 
        "admin");
        $this -> n = array();
        echo "Se conecto a la base de datos";
        }
    catch( PDOException $exp){
        echo ("No se logro conectar a la base de datos correctamente, $exp");
        }
    }

    public function get_usuario(){
        $sql="SELECT * FROM usuario 
            LEFT JOIN rol 
            ON usuario.rol_usuario=rol.id_rol
            LEFT JOIN estatus
            ON usuario.estatus=estatus.id_estatus
            RIGHT JOIN personal
            ON personal.usuario_personal=usuario.id_usuario";
        foreach ($this->conn->query($sql) as $row){
            $this->n[]=$row;
        }
            return $this->n;
            $this->conn=null;
    }

    public function get_rol(){
        $sql1="SELECT * FROM rol ; " ;
        foreach ($this->conn->query($sql1) as $row){
            $this->c[]=$row;
        }
            return $this->c;
            $this->conn=null;
    }

    public function get_usuario_por_id($id){
        $sql="SELECT * FROM usuario WHERE id_usuario= ? ;";
        $stmt = $this->conn->prepare ($sql);

        if ($stmt->execute(array($id) ))
        {
           while($row = $stmt->fetch())
           {
            $this->n[]=$row;
            }
        }
            return $this->n;
            $this->conn=null;
    }

    public function create_usuario(){
        print_r($_POST);
        if(empty($_POST["Estatus"]) or 
        empty($_POST["Contrasenia"]) or 
        empty($_POST["Contrasenia2"]) or 
        empty($_POST["Nom_usuario"]) or 
        empty($_POST["Tipo_de_usuario"]) or
        empty($_POST["Nombre"]) or 
        empty($_POST["Apellido"]) or 
        empty($_POST["Cedula"]) or 
        empty($_POST["Direccion"]) or 
        empty($_POST["Telefono"]) or 
        empty($_POST["Correo"]) or
        ($_POST["Contrasenia"] !== $_POST["Contrasenia2"])){
                header("location: ?m=1");
            exit;
        }
        else
        $sql = "INSERT INTO usuario VALUES (default,?,?,?,?);";
        $stmt = $this->conn->prepare ($sql);
        $stmt -> BindValue(1, $_POST["Estatus"], PDO::PARAM_STR);
        $stmt -> BindValue(2, $_POST["Contrasenia"], PDO::PARAM_STR);
        $stmt -> BindValue(3, $_POST["Nom_usuario"], PDO::PARAM_STR);
        $stmt -> BindValue(4, $_POST["Tipo_de_usuario"], PDO::PARAM_STR);
        $stmt -> execute();

        $sql1 = "INSERT INTO personal VALUES (default,?,?,?,?,?,?,?);";
        $stmt1 = $this->conn->prepare ($sql1);
        $stmt1 -> BindValue(1, $_POST["Nombre"], PDO::PARAM_STR);
        $stmt1 -> BindValue(2, $_POST["Apellido"], PDO::PARAM_STR);
        $stmt1 -> BindValue(3, $_POST["Cedula"], PDO::PARAM_STR);
        $stmt1 -> BindValue(4, $_POST["Direccion"], PDO::PARAM_STR);
        $stmt1 -> execute();

        $sql2 = "INSERT INTO numero_telefonico VALUES (default,?);";
        $stmt2= $this->conn->prepare ($sql2);
        $stmt2 -> BindValue(1, $_POST["Telefono"], PDO::PARAM_STR);
        $stmt2 -> execute();

        $sql3 = "INSERT INTO correo_electronico VALUES (default,?);";
        $stmt3= $this->conn->prepare ($sql3);
        $stmt3 -> BindValue(1, $_POST["Correo"], PDO::PARAM_STR);
        $stmt3 -> execute();
        $this->conn=null;
        header("location: ?m=2");
    }
    public function edit_usuario(){
        print_r($_POST);
        if(empty($_POST["Estatus"]) or empty($_POST["Contrasenia"]) or empty($_POST["Nom_usuario"]) or empty($_POST["Tipo_de_usuario"])){
            header("location: ?m=1&id=".$_POST["id"]);
            exit;
        }
        else
         $sql="UPDATE usuario
            SET
            estatus=?,
            contrasenia=?,
            nom_usuario=?,
            tipo_de_usuario=?
            WHERE
            id_usuario=?;
            ";
        $stmt = $this->conn->prepare ($sql);
        $stmt -> BindValue(1, $_POST["Estatus"], PDO::PARAM_STR);
        $stmt -> BindValue(2, $_POST["Contrasenia"], PDO::PARAM_STR);
        $stmt -> BindValue(3, $_POST["Nom_usuario"], PDO::PARAM_STR);
        $stmt -> BindValue(4, $_POST["Tipo_de_usuario"], PDO::PARAM_LOB);
        $stmt -> BindValue(7, $_POST["id"], PDO::PARAM_INT);
        
        $stmt -> execute();
        $this->conn=null;
        header("location: ?m=2&id=".$_POST["id"]);
    }

    public function delete_producto($id){
        $sql="DELETE  FROM usuario WHERE id_usuario=?";
        $stmt = $this->conn->prepare ($sql);
        $stmt ->BindParam(1,$id);
        $stmt -> execute();
        $this->conn=null;
        header("location: ?m=1");
    }

}
?>